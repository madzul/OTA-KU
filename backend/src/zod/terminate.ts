...
export const listTerminateForAdminResponse = z.object({
  ...
  message: z.string().openapi({ example: "Daftar request terminasi untuk Admin berhasil diambil" }),
  body: z.object({
    data: z.array(
      z.object({
        otaId: z
        ...
        createdAt: z.string().openapi({ example: "2025-05-12 05:38:29.984502"}),
...
export const terminationStatusMA = z.object({
  success: z.boolean().openapi({ example: true }),
  message: z.string().openapi({ example: "Status terminasi untuk MA berhasil diambil" }),
  body: z.object({
    otaId: z
        .string({
          required_error: "ID orang tua asuh harus diisi",
